...
namespace Rent.Repositories
{
    public class LocationRepository
    {
        ...
        public LocationRepository(RentContext context, LocationContext locationContext, UserContext userContext, LocationUserContext locationUserContext, DocumentRepository documentRepository, PropCondition propCondition, IRoleAuthenticationRepository roleAuthenticationRepository, FirestoreCommunicationRepository firestoreRepository)
        {
            _context = context;
        ...
        public async Task<int> Add(int requester, int customerId, Location location)
        {
            ...
            newLocation.FirestoreConversationStaff = await _firestoreRepository.CreateConversation(requester, true, location.Name + " - Staff", new List<User>());
        ...
        public async Task AddUserToLocation(int requester, int locationId, int userId)
        {
            ...
            //if (!_roleAuthenticationRepository.IsClientAdminOrClientManager(userId))
            //    await _firestoreRepository.AddUserToConversation(requester, location.FirestoreConversationStaff, user);
        }
        ...
        public async Task RemoveUserFromLocations(int requester, ICollection<int> locationIDs, int userId)
        {
            ...
            foreach (int lId in locationIDs)
            {
                var location = _context.Location.Find(lId);
                //await _firestoreRepository.RemoveUserFromConversation(requester, location.FirestoreConversationStaff, userId);
            }
        }
        ...
        public async Task Update(int requester, int locationId, Location ul)
        {
            await _locationContext.Update(requester, l => l.ID == locationId, l =>
            {
