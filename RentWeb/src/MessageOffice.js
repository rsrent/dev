...
class MessageOffice extends Component {


  constructor(props) {
      super(props);

      this.state = {comment: "", locationID: props.match.params.locationID, navigate: false};
  ...
  sendMessage(event) {
      ...
      HelperMethods.getRequest("Email/MailToOfficeFromWeb/" + this.props.match.params.locationID + "/" + comment, myHeaders,
          () => {
  ...
  render() {
      ...
      if (this.state.navigate) {
        return <Redirect to={this.props.location.pathname.substring(0, this.props.location.pathname.length - 8)} push={true} />
      }
              ...
              <div className="quality-report-card" id="quality-report-card1">
                  <div className="quality-report-card-header">
                      <div className="quality-report-card-title">
                          <h2>Send os en besked, så vil vi reagerer så hurtigt som muligt</h2>
                      </div>
                  </div>
                  {/* COMMENT */}
