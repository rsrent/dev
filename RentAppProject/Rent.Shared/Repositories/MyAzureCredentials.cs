...
//using ModuleLibraryShared.Storage;
...
namespace RentApp.Shared.Repositories
{
    public class MyAzureCredentials : AzureCredentials
    {
        ...
        public MyAzureCredentials(DocumentRepository documentRepository, Settings settings)
        {
        ...
        {
            var sas = (_settings.Live ? connectionString : connectionStringDev) + container + (await GetString(false, container));
        ...
        {
            var sas = (_settings.Live ? connectionString : connectionStringDev) + container + (await GetString(true, container));
            System.Diagnostics.Debug.WriteLine(sas);
            return sas;
        }
